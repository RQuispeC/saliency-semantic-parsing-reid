...
def main():
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'xent'}, use_gpu=use_gpu, mid_layer = args.mid_layer)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(epoch, model, criterion, optimizer, trainloader, use_gpu,  use_salience, use_parsing)
...
def train(epoch, model, criterion, optimizer, trainloader, use_gpu, use_salience, use_parsing):
    model.train()
    losses = AverageMeter()
    ...
    for batch_idx, tuple_i in enumerate(trainloader):
        ...
        if use_salience and not use_parsing:
            ...
            outputs = model(imgs, salience_masks = salience_imgs, parsing_masks = parsing_imgs)
        else:
            outputs = model(imgs)
